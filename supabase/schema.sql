...
create type order_status  as enum (
  'draft',          -- черновик менеджера (расчёт сделан, не утверждён)
  'pending',        -- на утверждении у Светы/Наташи
  'approved',       -- утверждён, ткань бронируется
  'fabric_needed',  -- ткани не хватает, нужна закупка
  'in_progress',    -- швея шьёт
  'sewn',           -- отшито, ждёт подтверждения расхода
  'done',           -- расход подтверждён, ткань списана
  'cancelled'
);
...
create table color_suppliers (                   -- у кого можно купить цвет
...
create view v_stock_by_color as
...
create view v_seamstress_load as
select s.id, s.name,
